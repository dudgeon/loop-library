...
Bring this kit's **machinery** up to date with canonical brainkit — **selectively**. Sync is a
...
> way `distill` curates the graph. Sync is non-deterministic by design.

## Inputs
- **origin** — where canonical upstream lives: the first thing the user hands you, else
  `$BRAINKIT_ORIGIN`, else the `origin` field in `loop.manifest.json`. A git URL or a local path; the
  kit may sit in a subdirectory (e.g. `dist/brainkit`). Point it at brainkit's **own** upstream — never
  a sibling kit (loopkit/karpathy-core), or you'd pull a sibling's skills over brainkit's.
- **machinery list** — `loop.manifest.json` → `managed_files`: the files upstream owns (the skills,
  this `CLAUDE.md`, `FOUNDATION.md`, `DESIGN.md`, the machinery READMEs, `work/templates/`). Everything
  else is the user's and is **out of scope** — never propose changing it: `PROJECT.md`, their
  `knowledge/` notes and `golden/`, their `knowledge/templates/` type notes (the starters are
  seed-once), their `work/` deliverables.
...
3. **Reason about value — don't dump diffs.** For each upstream-only / both-changed file, say in a
   ...
   this kit up (read `PROJECT.md` and their templates). A bugfix to a skill usually is; a doc reword may
   not be. This judgment is the whole reason sync is an agent and not `cp`.
