...
Bring this kit's **machinery** up to date with canonical loopkit — **selectively**. Sync is a
...
- **origin** — where canonical upstream lives: the first thing the user hands you, else
  `$LOOPKIT_ORIGIN`, else the `origin` field in `loop.manifest.json`. A git URL or a local path; the kit
  may sit in a subdirectory (e.g. `dist/loopkit`). Point it at loopkit's **own** upstream — never a
  sibling kit (karpathy-core/brainkit), or you'd pull a sibling's skills over loopkit's.
- **machinery list** — `loop.manifest.json` → `managed_files`: the files upstream owns (the skills,
  this `CLAUDE.md`, `FOUNDATION.md`, the machinery READMEs, `work/templates/`). Everything else is the
  user's and is **out of scope** — never propose changing it: `PROJECT.md`, their `knowledge/` notes and
  `golden/`, their `knowledge/templates/` type notes, their `work/` deliverables.
