...
- **Types emerge, then get written down** as type templates under `knowledge/templates/` — the folder
  a Duo vault reads types from, also read by the kit's skills (not locked — the assistant can refine
  them). A type's ordered status ladder lives there too. A fresh fork ships zero types and stays flat.
