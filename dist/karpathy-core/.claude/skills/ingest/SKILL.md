...
2. Decide where it belongs. Default: a new one-topic note in `knowledge/`. If it extends or
   duplicates an existing note, update that note instead of adding a near-duplicate. When it's a
   close call that matters, confirm with the structured **AskUserQuestion** prompt (`CLAUDE.md` §10).
