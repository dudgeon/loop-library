...
*(If your repo already has a `CLAUDE.md`, ask the assistant to merge karpathy-core's `CLAUDE.md` into it
...
karpathy-core packages a simple, proven idea: Andrej Karpathy's "LLM wiki," where an assistant keeps a
...
and karpathy-core is a way to make that kind of work easy to set up and keep running.

To pull upstream improvements into the kit's machinery later, ask the assistant to **sync** — it reads
what's changed in canonical karpathy-core and curates the compelling changes *with* you, selectively,
never clobbering the tweaks you've made on purpose.
